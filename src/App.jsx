...
const generateEditorialImage = async (base64Image, posePrompt) => {
  ...
  const payload = {
    ...
    generationConfig: {
      ...
      temperature: 0.9 // Creativity high for fashion
    }
  };
...
export default function LuminaApp() {
  ...
  const processImages = async (base64Input) => {
    ...
    // We generate 4 images. To avoid rate limits and ensure quality, we'll do them 2 at a time or sequentially.
    // Parallel requests might hit limits on the preview tier, so we'll do sequential for safety + visual progress.
    ...
    // Auto-generate video after images are done
  ...
  const generateGlamCamVideo = async (images) => {
    ...
    // Video dimensions (Portrait for social)
    const width = 720;
    const height = 1280; 
    canvas.width = width;
    ...
    // Animation Constants
    const durationPerImage = 60; // frames (approx 2 seconds)
    ...
    const animate = () => {
      ...
      if (currentImg) {
        ...
        // Calculate scale (slow zoom in)
        const maxScale = 1.2;
        const scale = 1.0 + (0.2 * (localFrame / durationPerImage));
        
        // Calculate dimensions based on aspect ratio to cover the canvas
  ...
  const handleShare = async () => {
    if (navigator.share && videoUrl) {
      try {
        // Need to convert blob URL to file for sharing in some browsers
